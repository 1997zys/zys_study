...
def get_predictions(params, model, data, vae = 'nothing', vaegt = None, prior=None):
    ...
    if vae == 'vae_vector':
        ...
        vaegt.eval()
        vaegt.load_state_dict(torch.load(params.vae_path,map_location='cpu'))  # vaegt.load_state_dict(torch.load(params.vae_path))
        aug_features, aug_ys = vf.augment_tensor(params, vaegt, support_features, support_ys, no_samples=1)
...
if __name__ == '__main__':
    ...
    print(split)
    params.hallucinator_dir = 'D:/TFFCN_fewshot-main/gen_training'
    ...
    method = 'inductive_tensor' # 'inductive_tensor' is for the WACV paper    method = 'inductive_vector'
    # method = 'inductive_LR'
    if method == 'inductive':
