...
def intercept_tensor(X:Tensor,index:int):
    befor = X[:(index*10),:,:,:]
    after = X[(index+1)*10:,:,:,:]
    return torch.cat((befor,after),dim=0)


def contrastive_proto(args, vae, protos, no_samples=10):
    z = torch.randn([5, 256, 5, 5]).to(args.device)
    protos_repeat = torch.repeat_interleave(protos, repeats=no_samples, dim=0)
    z = torch.repeat_interleave(z, repeats=no_samples, dim=0)
    mse = nn.MSELoss()
    #minimize MSE between same protos
    hy = vae.conditioner(protos_repeat)
    h = torch.add(z, hy)
    anchor = F.interpolate(h, size = (7, 7), mode = 'bilinear')
    anchor = vae.decoder_my(anchor)
    loss = mse(anchor, protos_repeat)
    return loss
...
def augment_tensor(params, vaegt, support_features, support_ys, no_samples=500):
    z = torch.randn([5, 256, 5, 5]).to(params.device)
...
def get_prototypes_tensor(fs, ys):#定义原型张量的计算
    classes_list = torch.unique(ys)#.tolist()
    Y = ys.detach().cpu().numpy()
    prototypes = []
    for i in classes_list:
        idx = np.where(Y == i.item())
        tmp = torch.mean(fs[idx], dim=0, keepdim=True)  #均值
        prototypes.append(tmp)
    prototypes = torch.cat(prototypes, dim = 0)
    return prototypes, classes_list
