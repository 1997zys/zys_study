...
def adapt(params, model, X, Y, steps=25):
    model.load_state_dict(torch.load(params.vae_path,map_location='cpu'))
...
def meta_train_tensor(args, model, vae, datamgr):
    ...
    scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=10, gamma=0.5)
    vae.train()
    for j in range(2):  #这里j为迭代次数，即迭代50次（从0到49）  for j in range(50)
        data_loader = datamgr.get_data_loader(args.loadfile, aug=True)
        for idx, data in enumerate(data_loader):
            support_features, support_ys = get_features_res18(params = args, model = model, data = data, feat_tensor=True)#support_ys到底是什么？
            protos, _ = vf.get_prototypes_tensor(support_features, support_ys)  #原型张量的计算
            optimizer.zero_grad()
            # generate from outside so that gradients propagate
            loss_cp = vf.contrastive_proto(args, vae, protos)#这里就是定义损失
            # Train
            total_loss = loss_cp
            total_loss.backward()
            optimizer.step()
...
if __name__ == '__main__':
    ...
    args.model = 'resnet18'
    backbone_dir = '../resnet18_224_models'   #这个是啥路径？
    if args.model == 'resnet18':
        image_size = 224
        args.n_shots = 5
        datamgr = SetDataManager(args, image_size)  #这里定义了batch_size = 20即：每一类20张图去训练张量幻觉器
        split = 'base'
        args.loadfile = configs.data_dir[args.dataset] + split + '.json'
        iterations = args.lr_decay_epochs.split(',') #iterations:['30','40']
        args.lr_decay_epochs = list([])
        for it in iterations:
            args.lr_decay_epochs.append(int(it))
        #data_loader = datamgr.get_data_loader(args.loadfile, aug=False)
        args.file_path = '%s/checkpoints/%s/%s/best_model_kd.tar' % (backbone_dir, args.dataset, 'resnet18')#蒸馏后得到的模型权重的路径
        ...
        elif args.hallucinator_type == 'tensor':    #这里本文选择幻觉器类型为张量
            vae_res = VAE_res()
            vae_res.to(args.device)
        model = EmbeddingNet(args).to(args.device)

        save_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), args.dataset, "vae", args.model)#save_dir：‘D:\\study\\TFH_fewshot_main\\gen_training\\miniImageneet\\vae\\resnet18

        if not os.path.isdir(save_dir):#如果要保存的路径不存在，则创建该路径
